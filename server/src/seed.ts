...
async function seed() {
    ...
    // Check if already seeded but check groups too
    const [users] = await pool.query('SELECT COUNT(*) as count FROM users') as any
    const [groups] = await pool.query('SELECT COUNT(*) as count FROM study_groups') as any

    if (users[0].count > 0 && groups[0].count > 0) {
    ...
    // Study Groups
    await pool.query(
        `INSERT INTO study_groups (id, name, description, type, created_by) VALUES
        (1, 'CS 6th Sem - Section A', 'Official class group for Section A', 'class', 2),
        (2, 'DSA Mentorship with Dr. Sharma', 'Doubt clearing and interview prep', 'mentorship', 2),
        (3, 'Web Dev Enthusiasts', 'Discussing latest frontend and backend tech', 'subject', 1)`
    )
    console.log('  ✓ Study Groups')

    // Group Members (User 1 is student, User 2 is Faculty, User 4 is Alumni)
    await pool.query(
    ...
    await pool.query(
        ...
        (1, 2, 'Welcome to the Section A group! I will post all urgent updates here.'),
        (1, 1, 'Thank you Dr. Sharma!'),
        (1, 2, 'Reminder: Your CS301 assignment is due next week.'),
        (2, 2, 'Let me know if anyone has doubts on the dynamic programming lecture.'),
        (2, 1, 'Could you share some practice problems for DP?'),
        (3, 1, 'Has anyone tried React 19 yet?'),
        (3, 4, 'Yes, the new hooks are great. I can share some code snippets we use at Google.')`
    )
